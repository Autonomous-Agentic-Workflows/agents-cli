...
def test_cmd_deploy_no_heavy_eager_imports():
    """Verify that importing google.agents.cli.deploy.cmd_deploy does not eagerly load heavy Vertex AI packages."""
    code = """
import sys
import google.agents.cli.deploy.cmd_deploy

heavy_modules = ["vertexai", "pandas", "google.cloud.resourcemanager_v3"]
loaded = [m for m in heavy_modules if any(k == m or k.startswith(m + ".") for k in sys.modules)]
if loaded:
    print(f"Error: Heavy modules loaded eagerly on import of cmd_deploy: {loaded}")
    sys.exit(1)
print("OK")
"""
    result = subprocess.run(
