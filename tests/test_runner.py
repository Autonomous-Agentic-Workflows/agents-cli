...
def test_redact_cmd_case_insensitive_and_extended():
    ...
    # Extended options (e.g. password, token, secret, bearer token, private key, service account key)
    ...
    args_5 = ["deploy", "--bearer-token", "bearer_token_xyz"]
    ...
    args_7 = ["deploy", "--service-account-key", "sa_json_secret"]
    assert redact_cmd(args_7) == "deploy --service-account-key '[REDACTED]'"

    # Case-insensitive env vars (e.g. lowercase) and new sensitive env vars
    ...
    args_10 = ["env", "BEARER_TOKEN=bearer_123", "python"]
    assert redact_cmd(args_10) == "env 'BEARER_TOKEN=[REDACTED]' python"

    args_11 = ["env", "PRIVATE_KEY=pk_123", "python"]
    assert redact_cmd(args_11) == "env 'PRIVATE_KEY=[REDACTED]' python"

    args_12 = ["env", "SERVICE_ACCOUNT_KEY=sa_123", "python"]
    assert redact_cmd(args_12) == "env 'SERVICE_ACCOUNT_KEY=[REDACTED]' python"

    args_13 = ["env", "API_KEY=key_123", "python"]
    assert redact_cmd(args_13) == "env 'API_KEY=[REDACTED]' python"
