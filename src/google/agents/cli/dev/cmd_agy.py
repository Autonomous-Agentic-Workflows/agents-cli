...
def bridge():
    ...
    # Load Vertex env script directly into env dict without shell=True
    if os.path.exists(VERTEX_ENV):
        try:
            with open(VERTEX_ENV, encoding="utf-8") as f:
                for line in f:
                    line = line.strip()
            ...
            click.echo(f"Warning: Failed to parse {VERTEX_ENV}: {e}")
