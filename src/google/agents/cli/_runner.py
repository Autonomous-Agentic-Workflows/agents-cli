...
def redact_cmd(args: list[str]) -> str:
    ...
    Masks arguments like --github-pat, --api-key, --authorization, HTTP headers,
    ...
    for i, raw_arg in enumerate(args):
        ...
        if arg_lower in _SENSITIVE_OPTIONS and i + 1 < len(args):
            ...
            redacted_cmd_list[i] = f"{arg[:7]}[REDACTED]"
        elif ":" in arg and any(
            arg_lower.startswith(h)
            for h in ("authorization:", "x-api-key:", "x-auth-token:")
        ):
            header, sep, val = arg.partition(":")
            space = " " if val.startswith(" ") else ""
            redacted_cmd_list[i] = f"{header}{sep}{space}[REDACTED]"
