...
def cmd_run(
    ...
    *,
    ...
    """
    # Deferred imports of heavy httpx/requests dependencies to optimize startup latency
    import httpx
    import requests
...
def _dispatch_query(
    ...
    *,
    ...
    """
    if mode == "a2a":
        # Deferred import of heavy a2a constant
...
def _print_author_tag(author: str | None, last_author: str | None) -> str | None:
    ...
    if author and author != last_author:
        if last_author is not None:
            click.echo()
        click.secho(f"[{author}]: ", fg="green", bold=True, nl=False)
        return author
    return last_author
...
def _query_adk_sse(
    ...
    *,
    ...
    if user_text:
        click.secho("[user]: ", fg="yellow", bold=True, nl=False)
...
async def _query_a2a_async(
    ...
    from a2a.types import Message, Role, TaskArtifactUpdateEvent, TextPart, TransportProtocol
...
def _print_a2a_part(part: Part, artifacts: list[str]) -> None:
    ...
    if isinstance(root, TextPart) and root.text:
        ...
        if isinstance(file_data, FileWithUri):
            click.secho(f"\n[file: {file_data.uri}]", fg="cyan", nl=False)
...
def _print_sse_part(part: dict, artifacts: list[str]) -> None:
    ...
    if function_response:
        ...
        click.secho(
            f"\n[tool_response: {name} -> {json.dumps(response)}]",
            fg="magenta",
            nl=False,
        )
        return
