...
@click.option(
    ...
    is_flag=True,
    default=True,
    help="Enable interactive authentication (default).",
)
@click.option(
    "--status",
    is_flag=True,
...
def cmd_login(interactive: bool, status: bool):
    ...
    if status:
        ...
        if authed:
            ...
            click.secho("    Run 'agents-cli login' to authenticate.", dim=True)
        click.echo()
        return

    if not interactive:
        raise click.UsageError(
            "'login' requires interactive mode. Run without '--no-interactive' to authenticate."
        )
    click.echo()
