...
def run_auth_step(show_header=True):
    ...
    click.echo()
    click.secho("  1. Google Cloud (ADC)", fg="cyan", bold=True)
    click.echo("     Use gcloud Application Default Credentials.")
    click.secho("     Best for: production workloads, full GCP access", dim=True)
    click.echo()
    click.secho("  2. Gemini API Key", fg="cyan", bold=True)
    click.echo("     Enter an API key from AI Studio (aistudio.google.com).")
    click.secho("     Best for: getting started, personal projects", dim=True)
    click.echo()
