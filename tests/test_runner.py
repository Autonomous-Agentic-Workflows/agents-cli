...
def test_redact_cmd_case_insensitive_and_extended():
    ...
    # Extended options (e.g. password, token, secret, authorization, refresh-token, bearer-token)
    ...
    args_5 = ["curl", "-H", "--authorization", "Bearer secret_token_abc"]
    assert redact_cmd(args_5) == "curl -H --authorization '[REDACTED]'"

    args_6 = ["deploy", "--refresh-token=refresh_12345"]
    assert redact_cmd(args_6) == "deploy '--refresh-token=[REDACTED]'"
...
def test_redact_cmd_http_headers_and_bearer_tokens():
    ...
    args_3 = ["curl", "-H", "Bearer my_standalone_token", "https://example.com"]
