...
"""Tests for agents-cli login command and interactive authentication defaults."""
...
def test_login_status_authenticated():
    """Verify login --status when already authenticated."""
    runner = CliRunner()
    with patch("google.agents.cli.setup.cmd_auth.is_authenticated", return_value=(True, "Test Creds")):
        result = runner.invoke(cmd_login, ["--status"])
        assert result.exit_code == 0
        assert "Status" in result.output
        assert "Authenticated as Test Creds" in result.output
...
def test_login_default_interactive():
    """Verify login command defaults to interactive mode and runs the auth step."""
    runner = CliRunner()
...
def test_login_no_interactive_error():
    """Verify login command fails with a UsageError when --no-interactive is passed."""
    runner = CliRunner()
    with patch("google.agents.cli.setup.cmd_auth.run_auth_step") as mock_run_auth_step:
