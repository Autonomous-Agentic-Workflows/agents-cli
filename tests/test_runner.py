...
def test_redact_cmd_headers_and_bearer_tokens():
    ...
    args_1 = ["curl", "-H", "Bearer secret-jwt-token-xyz", "https://example.com"]
    assert redact_cmd(args_1) == "curl -H 'Bearer [REDACTED]' https://example.com"

    # Authorization header
    args_2 = ["curl", "-H", "Authorization: Bearer secret-jwt-token-xyz"]
    assert redact_cmd(args_2) == "curl -H 'Authorization: [REDACTED]'"

    # X-Api-Key header
    args_3 = ["curl", "-H", "X-Api-Key: AIzaSyKey123"]
    assert redact_cmd(args_3) == "curl -H 'X-Api-Key: [REDACTED]'"

    # Additional sensitive options (e.g. --bearer-token, --private-key)
    args_4 = ["deploy", "--bearer-token", "my-bearer-token"]
    assert redact_cmd(args_4) == "deploy --bearer-token '[REDACTED]'"

    args_5 = ["deploy", "--private-key=secret-key-content"]
